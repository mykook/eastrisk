...
parse_event('ChannelUpdate', Elements) ->
...
ast_state_record([<<"State: ", State/binary>>|T], Record, ActionID) ->
    StateDesc = binary_to_atom(State),
    ast_state_record(T, Record#ast_state{state = StateDesc, state_desc = StateDesc}, ActionID);
ast_state_record([<<"ChannelState: ", State/binary>>|T], Record, ActionID) ->
    ast_state_record(T, Record#ast_state{state = list_to_integer(binary_to_list(State))}, ActionID);
ast_state_record([<<"ChannelStateDesc: ", State/binary>>|T], Record, ActionID) ->
    ast_state_record(T, Record#ast_state{state_desc = binary_to_atom(State)}, ActionID);
ast_state_record([<<"CallerID: ", CallerID/binary>>|T], Record, ActionID) ->
    ast_state_record(T, Record#ast_state{caller_id = binary_to_list(CallerID)},
        ActionID);
ast_state_record([<<"CallerIDNum: ", CallerID/binary>>|T], Record, ActionID) ->
    ast_state_record(T, Record#ast_state{caller_id = binary_to_list(CallerID)},
        ActionID);
...
channel_record([<<"Channel: ", Channel/binary>>|T], Record, ActionID) ->
    channel_record(T, Record#channel{channel = binary_to_list(Channel)}, 
                   ActionID);
channel_record([<<"State: ", State/binary>>|T], Record, ActionID) ->  %% for Asterisk 1.2
    StateDesc = binary_to_atom(State),
    channel_record(T, Record#channel{state = StateDesc, state_desc = StateDesc}, ActionID);
channel_record([<<"ChannelState: ", State/binary>>|T], Record, ActionID) -> 
    channel_record(T, Record#channel{state = list_to_integer(binary_to_list(State))}, ActionID);
channel_record([<<"ChannelStateDesc: ", State/binary>>|T], Record, ActionID) ->
    channel_record(T, Record#channel{state_desc = binary_to_atom(State)}, ActionID);
...
channel_record([<<"AccountCode: ", Code/binary>>|T], Record, ActionID) ->
    channel_record(T, Record#channel{account_code = binary_to_list(Code)},
                   ActionID);
channel_record([<<"Exten: ", Exten/binary>>|T], Record, ActionID) ->
    channel_record(T, Record#channel{exten = binary_to_list(Exten)},
                   ActionID);
channel_record([<<"Context: ", Context/binary>>|T], Record, ActionID) ->
    channel_record(T, Record#channel{context = binary_to_list(Context)},
                   ActionID);
...
hangup_record([<<"CallerIDNum: ", CallerId/binary>>|T], Record, ActionID) ->
